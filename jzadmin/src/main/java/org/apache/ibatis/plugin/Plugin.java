...
import com.caiyi.financial.jzadmin.plugin.PagePlugin;
import com.caiyi.financial.jzadmin.plugin.PageView;
import com.caiyi.financial.jzadmin.util.Common;
import com.caiyi.financial.jzadmin.util.EhcacheUtils;
	...
	public static final String joinSql(Connection connection, MappedStatement mappedStatement, BoundSql boundSql,
		...
		if (null != formMap) {
			...
			for (String key:formMap.keySet()){
				if (key.equals("ly_table")){
					convertMap.put(key, formMap.get(key));
				} else {
					convertMap.put(key.toUpperCase(), formMap.get(key)); //key转为大写
				}
			...
			mapfield = (Map<String, Object>) EhcacheUtils.get(tableName.toUpperCase());
