package com.caiyi.financial.jzadmin.shiro;
...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		if (loginName != null) {
			...
			for (ResFormMap resources : rs) {
				info.addStringPermission(resources.get("resKey".toUpperCase()).toString());
			}
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		if (userFormMaps.size() != 0) {
			if ("2".equals(userFormMaps.get(0).get("locked".toUpperCase()))) {
			...
			SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(username, // 用户名
					userFormMaps.get(0).get("password".toUpperCase()), // 密码
					ByteSource.Util.bytes(username + "" + userFormMaps.get(0).get("credentialsSalt".toUpperCase())),// salt=username+salt
					getName() // realm name
			);
			// 当验证都通过后，把用户信息放在session里
			Session session = SecurityUtils.getSubject().getSession();
			session.setAttribute("userSession", userFormMaps.get(0));
			session.setAttribute("userSessionId", userFormMaps.get(0).get("id".toUpperCase()));
